...
const TeamNameView = (props: { team?: API.MatchTeam }) => {
  return (
    <Flex justify={"space-between"} align="center" w="100%">
      <Text>{props.team?.name ?? "Nog niet bepaald"}</Text>
    ...
    return (
      ...
      <SeedItem style={{ backgroundColor: isFinal ? theme.colors.yellow[7] : theme.colors.vek[7] }}>
          <div>
            <SeedTeam>
              {readonly ? (
                <TeamNameView team={seed.teams[0] as API.MatchTeam | undefined} />
              ) : (
                <BracketTeamInput
                  team={seed.teams[0] as API.MatchTeam}
                  onTeeamSelection={updateMatchTeams}
                  matchId={Number(matchInfo?.id)}
                  disabled={matchInfo && loading[Number(matchInfo.id)]}
                />
              )}
            </SeedTeam>
            <SeedTeam>
              {readonly ? (
                <TeamNameView team={seed.teams[1] as API.MatchTeam | undefined} />
              ) : (
                <BracketTeamInput
                  team={seed.teams[1] as API.MatchTeam}
                  onTeeamSelection={updateMatchTeams}
                  matchId={Number(matchInfo?.id)}
                  disabled={matchInfo && loading[Number(matchInfo.id)]}
                />
            ...
            {!readonly && (
              <SeedTeam>
                <DateTimePicker
                  w={"100%"}
                  disabled={matchInfo && loading[Number(matchInfo.id)]}
                  defaultValue={new Date(seed.date ?? Date.now())}
                  onFocus={() => setUpdatingDate(new Date(seed.date ?? Date.now()))}
                  submitButtonProps={{ onClick: () => updateMatchDate() }}
                />
              </SeedTeam>
            )}
          </div>
